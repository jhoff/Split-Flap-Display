...
SplitFlapMqtt::SplitFlapMqtt(JsonSettings &settings, WiFiClient &wifiClient)
    : settings(settings), wifiClient(wifiClient), mqttClient(wifiClient),
      display(nullptr) {}
...
void SplitFlapMqtt::setup() {
  ...
  topic_config_sensor =
      "homeassistant/sensor/splitflap_sensor_" + mdns + "/config";
